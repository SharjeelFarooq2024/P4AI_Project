...
class PreprocessingPipeline:
    ...
    def transform(self, X: pd.DataFrame, apply_imbalance_handling: bool = False,
        ...
        # Only scale columns that exist in incoming data (helps inference when capture schema is narrower)
        if self.wide_range_cols_:
            present_cols = [c for c in self.wide_range_cols_ if c in X.columns]
            if present_cols:
                X[present_cols] = self.scaler.transform(X[present_cols])
        else:
            # scaler.scaled_columns_ already accounts for training cols; intersect with incoming
            present_cols = [c for c in self.scaler.scaled_columns_ if c in X.columns]
            if present_cols:
                X[present_cols] = self.scaler.transform(X[present_cols])
    ...
    def run_full_pipeline(self, apply_imbalance: bool = True):
        ...
        # For training - drop both target and attack_cat (attack_cat is for multi-class, we use binary label)
